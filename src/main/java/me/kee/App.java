...
import me.kee.util.CookieUtil;
import me.kee.util.Matchers;
...
public class App {
	...
	public String showQrCode() {
		...
		final File output = new File("tmp.jpg");
		
		HttpRequest.post(url, true, 
				"t", "webwx", 
				"_" , DateKit.getCurrentUnixTime()).receive(output);
		
		return url+"?t=webwx&"+"_"+DateKit.getCurrentUnixTime();
	}
	...
	public void handleMsg(JSONObject data){
		...
		for(int i=0,len=AddMsgList.size(); i<len; i++){
			...
			if(msgType == 51){
				...
				webwxsendmsg("金航太笨了，还不支持图片呢", msg.getString("FromUserName"));
			} else if(msgType == 34){
				webwxsendmsg("金航太笨了，还不支持语音呢", msg.getString("FromUserName"));
			} else if(msgType == 42){
	...
	// 这里的api_key=8431f2507b17da12672c6956e914deb1和api_secret=hh56xwzny2b2可以自己申请一个
	private final String KEY = "?api_key=8431f2507b17da12672c6956e914deb1&api_secret=hh56xwzny2b2";
